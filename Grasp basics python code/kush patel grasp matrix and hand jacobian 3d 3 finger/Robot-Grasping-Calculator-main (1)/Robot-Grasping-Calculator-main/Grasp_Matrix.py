...
contact_points = [[-1.0, -0.5, 0.0], 
                  [-1.0, 0.5, 0.0]
                  [1.0, 0.0, 0.0]]  # Example: Contact points on the sphere (w.r.t. body frame)
...
if k > len(contact_points):
    ...
    if m == 6 and p != 3: # After calculating the full grasp matrix, check the dimensionality (m)
