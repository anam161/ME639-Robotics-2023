...
#ranges of motion for index
theta1_index_range = np.linspace(np.deg2rad(60), np.deg2rad(90), 30)  # Joint 1: Flexion/Extension
theta2_index_range = np.linspace(np.deg2rad(-5), np.deg2rad(5), 30)  # Joint 2: Abduction/Adduction
theta3_index_range = np.linspace(0, np.deg2rad(20), 30)  # Joint 3: Flexion/Extension
theta4_index_range = np.linspace(0, np.deg2rad(10), 30)  # Joint 4: Flexion/Extension

theta1i, theta2i, theta3i, theta4i = np.meshgrid(theta1_index_range, theta2_index_range, theta3_index_range, theta4_index_range)
theta1T, theta2T, theta3T, theta4T = np.meshgrid(theta1_thumb_range, theta2_thumb_range, theta3_thumb_range, theta4_thumb_range)

# Create a meshgrid of all possible joint angles for the thumb
theta1T, theta2T, theta3T, theta4T = np.meshgrid(
...
x1_index = L1 * np.cos(theta1i)
y1_index = L1 * np.sin(theta1i)
z1_index = np.random.uniform(-1e-3, 1e-3, x1_index.shape)+4 # Small random perturbation
...
x2_index = L1 * np.cos(theta1i) * np.cos(theta2i) + PIP * np.cos(theta1i + theta3i) * np.cos(theta2i)
y2_index = L1 * np.sin(theta1i) + PIP * np.sin(theta1i + theta3i)
z2_index = L1 * np.cos(theta1i) * np.sin(theta2i) + PIP * np.cos(theta1i + theta3i) * np.sin(theta2i)+4
...
x4_index = (L1 * np.cos(theta1i) * np.cos(theta2i) + 
            PIP * np.cos(theta1i + theta3i) * np.cos(theta2i) + 
            L4 * np.cos(theta1i + theta3i + theta4i) * np.cos(theta2i))
y4_index = (L1 * np.sin(theta1i) + 
            PIP * np.sin(theta1i + theta3i) + 
            L4 * np.sin(theta1i + theta3i + theta4i))
z4_index = (L1 * np.cos(theta1i) * np.sin(theta2i) + 
            PIP * np.cos(theta1i + theta3i) * np.sin(theta2i) + 
            L4 * np.cos(theta1i + theta3i + theta4i) * np.sin(theta2i)+4)
...
z1_pinky = np.random.uniform(-1e-3, 1e-3, x1_pinky.shape)   # Small random perturbation
...
z2_pinky = L1 * np.cos(theta1) * np.sin(theta2) + PIP * np.cos(theta1 + theta3) * np.sin(theta2) 
...
x1_thumb = L1_thumb * np.cos(theta1T) * np.cos(theta2T)-3.2
y1_thumb = L1_thumb * np.sin(theta1T)
z1_thumb = L1_thumb * np.cos(theta1T) * np.sin(theta2T)+10
...
x2_thumb = L1_thumb * np.cos(theta1T) * np.cos(theta2T) + L2_thumb * np.cos(theta1T + theta3T) * np.cos(theta2T)-3.2
y2_thumb = L1_thumb * np.sin(theta1T) + L2_thumb * np.sin(theta1T + theta3T)
z2_thumb = L1_thumb * np.cos(theta1T) * np.sin(theta2T) + L2_thumb * np.cos(theta1T + theta3T) * np.sin(theta2T) + 10
...
x3_thumb = (L1_thumb * np.cos(theta1T) * np.cos(theta2T) + 
            L2_thumb * np.cos(theta1T + theta3T) * np.cos(theta2T) + 
            L3_thumb * np.cos(theta1T + theta3T + theta4T) * np.cos(theta2T)-3.2)
y3_thumb = (L1_thumb * np.sin(theta1T) + 
            L2_thumb * np.sin(theta1T + theta3T) + 
            L3_thumb * np.sin(theta1T + theta3T + theta4T))
z3_thumb = (L1_thumb * np.cos(theta1T) * np.sin(theta2T) + 
            L2_thumb * np.cos(theta1T + theta3T) * np.sin(theta2T) + 
            L3_thumb * np.cos(theta1T + theta3T + theta4T) * np.sin(theta2T) + 10)
...
finger_origin_index = [0,0,4]  # Positioning the model just above the reachability space
...
finger_origin_pinky = [0,0,0]  # Positioning the model just above the reachability space
...
plotter.add_mesh(cylinder, color='gray', opacity=1.0, show_edges=True)
