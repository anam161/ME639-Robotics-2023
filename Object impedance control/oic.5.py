...
def jacobian(l1, l2, l3, theta1, theta2, theta3, alpha=1):
    J = np.array([
        [-l1*np.sin(theta1) - l2*np.sin(theta1+theta2) - l3*np.sin(theta1+theta2+theta3),
         ...
         -alpha*l3*np.sin(theta1+theta2+theta3)],  # Scaling factor applied to the third column (third segment)
        [l1*np.cos(theta1) + l2*np.cos(theta1+theta2) + l3*np.cos(theta1+theta2+theta3),
         ...
         alpha*l3*np.cos(theta1+theta2+theta3)],  # Scaling factor applied to the third column (third segment)
        [0, 0, 0] 
    ])
    return J
...
J1 = jacobian(l1_f1, l2_f1, l3_f1, theta1_f1, theta2_f1, theta3_f1, alpha)
...
J2 = jacobian(l1_f2, l2_f2, l3_f2, theta1_f2, theta2_f2, theta3_f2, alpha)
...
def draw_axis(frame, camera_matrix, dist_coeffs, rvec, tvec, length=0.03):
...
def calculate_relative_pose(rvec1, tvec1, rvec2, tvec2):
    ...
    t_relative[2] -= 1.0 # Apply correction for Z drift
    R_relative = R2 @ R1.T
...
def get_palm_frame_transformation():
    ...
    translation_vector = np.array([0, 0, 0])  # Palm is 14.5cm above the object
...
def draw_origin_and_object_axes(frame, camera_matrix, dist_coeffs, rvec_object, tvec_object):
    ...
    frame = draw_axis(frame, camera_matrix, dist_coeffs, origin_rvec, origin_tvec, 0.03)  # Origin axes
    frame = draw_axis(frame, camera_matrix, dist_coeffs, rvec_palm, tvec_palm_frame, 0.03)  # Palm axes
    ...
    while True:
        ...
        if ids is not None:
            ...
            for i in range(len(ids)):
                rvec, tvec, _ = aruco.estimatePoseSingleMarkers(corners[i], 0.03, camera_matrix, dist_coeffs)
...
class LeapNode:
    ...
    def calculate_and_set_torque(self):
        ...
        Kp = np.eye(6) * 250   # Proportional gain, stiff spring to return to desired position
        Kd = np.eye(6) * 100 # Damping gain, smoothens the return
        ...
        N = np.array([0,0,3,0,0,3])  # Nullspace forces to maintain balance
...
def main(**kwargs):
    ...
    while True:
        ...
        print("pose: " + str(leap_hand.read_pos()) )
        time.sleep(0.5)
...
if __name__ == "__main__":
    ...
    torque_thread = threading.Thread(target=main)
